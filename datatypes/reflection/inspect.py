...
class ReflectFinder(ClassFinder):
    """Internal class. Used by `ReflectABC` to keep track of subclasses"""
    def __init__(self):
        self.lookup = {}
        super().__init__()

    def find_best_subclass(self, call_class: type, klass: type) -> type:
        """Overrides parent method to add caching"""
...
class ReflectABC(object):
    ...
    """Holds all the reflect class children"""
    ...
    @classmethod
    def find_reflect_class(cls, reflect_class: type) -> type:
        """Find the best subclass for `reflect_class` that is closest to
        the definition of `cls`"""
        return cls.classfinder.find_best_subclass(cls, reflect_class)

    def _create_reflect_instance(
        self,
        class_key: str,
        reflect_class: type,
        *args,
        **kwargs,
        ...
        """Internal method. This is used by all the `create_reflect_*`
        methods to create a new reflection instance.

        This does all the heavy lifting to figure out the best class to
        use to create the instance

        :param class_key: if a value is found in `kwargs` at this key then
            that class will be used instead of the inferrence class
        :param reflect_class: ReflectABC, we're looking for the best match
            of this reflection class
        :argument *args: passed through to reflection class's __init__
        :keyword **kwargs: these are passed through to the reflection class's
            __init__ method
        """
        if class_key in kwargs:
            reflect_class = kwargs[class_key]
...
class ReflectArgument(ReflectObject):
    ...
    def __init__(
        ...
        *,
        ...
        keyword_value: Any = inspect.Parameter.empty,
        **kwargs,
    ):
        """
        ...
        self.positional_value = positional_value
    ...
    def has_keyword_value(self) -> bool:
        ...
        """
        return self.get_keyword_value() is not inspect.Parameter.empty
...
class ReflectCallable(ReflectObject):
    ...
    def get_bind_info(self, *args, **kwargs):
        ...
        for ra in self.reflect_arguments(*args, **kwargs):
            ...
            if ra.is_bound():
                if ra.is_positional():
                    if ra.is_catchall():
                        ret["bound_args"].extend(ra.get_value())

                    else:
                        ret["bound_names"].add(ra.name)
                        ret["bound_args"].append(ra.get_value())

                elif ra.is_keyword():
                    if ra.is_catchall():
                        ret["bound_kwargs"].update(ra.get_value())

                    else:
                        ret["bound_names"].add(ra.name)
                        ret["bound_kwargs"][ra.name] = ra.get_value()

                if ra.has_multiple_values():
                    ret["multiple_names"].add(ra.name)

            else:
                if ra.has_positional_value():
                    ret["unbound_args"] = ra.get_value()

                elif ra.has_keyword_value():
                    ret["unbound_kwargs"] = ra.get_value()
    ...
    def reflect_arguments(self, *args, **kwargs):
            ...
            try:
                ...
                try:
                    ...
                    yield self.create_reflect_argument(
                        None,
                        positional_value=[arg_val] + list(arg_vals),
                    ...
                    if param.kind == param.VAR_POSITIONAL:
                        ...
                        # A positional value passed in as a positional
                        yield self.create_reflect_argument(
                            param,
        ...
        if param:
            # we haven't consumed the keyword param we ran into while
            # consuming the positionals so let's make sure we consume it
            params = itertools.chain([param], params)

        # we've consumed all the positional arguments so now we need to go
        # through the keyword arguments
        for param in params:
            if param.kind == param.VAR_KEYWORD:
                # found the **kwargs param, we'll consume this last, we do
                # this after this loop because we might have more kwargs but
                # no param, that's why we save this and actually consume it
                # later
                ...
                if kw_args := kwargs.pop(param.name, []):
                    # the positional argument (eg, `*args`) was passed in as a
                    # keyword (eg, `args=[...]`)
                    yield self.create_reflect_argument(
                        param,
                        kind=param.VAR_POSITIONAL,
                        keyword_value=kw_args,
                    )
        ...
        if kwargs:
            ...
            # the VAR_KEYWORD param or unbounded
            yield self.create_reflect_argument(
                keywords_param,
                kind=inspect.Parameter.VAR_KEYWORD,
                keyword_value=kwargs,
            )
