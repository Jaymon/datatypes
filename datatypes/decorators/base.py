    ...
    def __new__(cls, *args, **kwargs):
        ...
        if instance.is_possible_wrap_call(*args, **kwargs):
            ...
            if instance.is_class(args[0]):
                ...
                logger.debug("__new__ returning wrapped class")
                try:
                    ...
                    logger.debug("__new__ failed ambiguous class wrap")
                    instance.wrapped_call = "__call__"
...
#         wrapper.__wrapped__ = wrapped
    ...
    def wrap(self, wrapped, *decorator_args, **decorator_kwargs):
        if self.is_function(wrapped):
            logger.debug("Calling decorate_func()")
            ...
            logger.debug("Calling decorate_class()")
    ...
    def __get__(self, instance, instance_class):
        ...
        """
        logger.debug("__new__ was the wrap call because __get__ called")
    ...
    def __call__(self, *args, **kwargs):
        ...
        if self.wrapped_call == "__call__":
            ...
            if self.is_possible_wrap_call(*args, **kwargs):
                logger.debug("__call__ could be a wrap call")
                ...
                if is_possible:
                    logger.debug("__new__ could be a wrap call also")
                    ...
                    logger.debug(
                        "choosing __call__ as wrapped call over __new__"
                    )

                else:
                    logger.debug("choosing __call__ as wrapped call")
                    wrapped = args[0]
        ...
        if invoke:
            logger.debug("Invoking decorated value")
            try:
                ret = ret(*args, **kwargs)
